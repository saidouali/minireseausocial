...
if(!empty($_POST)){ 
  ...
  $valid =(boolean)true;
    ...
    if(isset($_POST['Modifier-profil'])){
       ...
       $departement = (String) htmlentities (trim($departement));
...
<option value="<?= $verif_departement['departement_code'] ?>"><?= $verif_departement['departement_nom']?></option>
    ...
    <?php
      if(isset($err_mail)){
        ...
        <input type="submit" name="Modifier-profil" value="Modifier" style="background: #D2B48C">
</form>
</div>
   </div>
     <div class="membre-corps" style="text-align: left">
      ...
      ?>
       ...
       ?>
            ...
            <br>
            <div>
              <input type="submit" name="envoyer" value="Envoyer le fichier" style="background: #D2B48C">
</div>    <br>       </form>
         </div>
      </div>  
    </div>
